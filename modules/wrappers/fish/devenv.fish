...
# Spawn a devenv shell for $project_dir (without leaving $PWD) and follow the
# user if they cd'd out.
function _devenv_hook_activate
    ...
    # No `env -C $project_dir` here: `devenv shell` resolves the project root by
    # walking up from $PWD (setting DEVENV_ROOT to it) while leaving $PWD alone.
    # Chdir'ing to the root would silently drop you there when the hook fires
    # from a subdirectory (e.g. cd ~/work/<repo> with devenv.nix in ~/work).
    # The `"$DEVENV_ROOT/*"` case in `_devenv_hook` still catches cd-ing out.
    env _DEVENV_HOOK_DIR=$project_dir _DEVENV_CALLER=hook _DEVENV_SHELL_HINT=fish devenv shell --no-tui --no-reload
